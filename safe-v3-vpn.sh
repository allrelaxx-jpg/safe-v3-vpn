cat > /root/install-firewall-selfheal-safe-v3-vpn.sh <<'EOF'
#!/usr/bin/env bash
set -Eeuo pipefail

INSTALL_LOG="/root/firewall-selfheal-install-v3-vpn-$(date +%F-%H%M%S).log"
...
BACKUP_DIR="/root/firewall-selfheal-backup-v3-vpn-$(date +%F-%H%M%S)"
...
show_failure_context() {
  ...
  echo "--- iptables FIREWALL-TRUSTED ---"
  iptables -S FIREWALL-TRUSTED || true
...
ensure_trusted_input_chain() {
  log "Проверка FIREWALL-TRUSTED"
  iptables -N FIREWALL-TRUSTED 2>/dev/null || true
  iptables -F FIREWALL-TRUSTED

  local ip
  for ip in "${NEVER_BLOCK_IPS[@]}"; do
    iptables -A FIREWALL-TRUSTED -s "$ip" -j ACCEPT
  done

  iptables -C INPUT -j FIREWALL-TRUSTED >/dev/null 2>&1 || iptables -I INPUT 1 -j FIREWALL-TRUSTED

  local first_rule
  first_rule="$(iptables -S INPUT | head -n 1 || true)"
  if [[ "$first_rule" != "-A INPUT -j FIREWALL-TRUSTED" ]]; then
    iptables -D INPUT -j FIREWALL-TRUSTED 2>/dev/null || true
    iptables -I INPUT 1 -j FIREWALL-TRUSTED
  fi

  log "FIREWALL-TRUSTED установлен первым в INPUT"
}
...
rebuild_docker_user() {
  ...
  for ip in "${ADMIN_IPS[@]}"; do
    iptables -A DOCKER-USER -p tcp -s "$ip" --dport 8080 -j ACCEPT
  done
...
verify_docker_user() {
  ...
  for ip in "${ADMIN_IPS[@]}"; do
    iptables_rule_exists -p tcp -s "$ip" --dport 8080 -j ACCEPT || ok=0
  done
...
selftest() {
  ...
  iptables -S FIREWALL-TRUSTED >/dev/null 2>&1 || { log "SELFTEST FAIL: FIREWALL-TRUSTED missing"; fail=1; }

  iptables -C INPUT -j FIREWALL-TRUSTED >/dev/null 2>&1 || {
    log "SELFTEST FAIL: INPUT does not jump to FIREWALL-TRUSTED"; fail=1; }
  ...
  for ip in "${NEVER_BLOCK_IPS[@]}"; do
    iptables -C FIREWALL-TRUSTED -s "$ip" -j ACCEPT >/dev/null 2>&1 || {
      log "SELFTEST FAIL: FIREWALL-TRUSTED missing ACCEPT for $ip"; fail=1; }
    iptables -C DOCKER-USER -s "$ip" -j ACCEPT >/dev/null 2>&1 || {
      log "SELFTEST FAIL: DOCKER-USER missing never-block ACCEPT for $ip"; fail=1; }
  done

  for ip in "${ADMIN_IPS[@]}"; do
    iptables -C DOCKER-USER -p tcp -s "$ip" --dport 8080 -j ACCEPT >/dev/null 2>&1 || {
      log "SELFTEST FAIL: missing 8080 whitelist for $ip"; fail=1; }
  done
...
iptables -S INPUT 2>/dev/null | grep -E 'FIREWALL-TRUSTED|ULTRA-SOC|GEO' || true
echo
echo "===== UFW STATUS ====="
ufw status numbered || true
echo
...
tail -n 150 "$LOG_FILE" || true
echo
green "INSTALL COMPLETE: firewall-selfheal-safe-v3-vpn installed successfully"
EOF

chmod +x /root/install-firewall-selfheal-safe-v3-vpn.sh
bash /root/install-firewall-selfheal-safe-v3-vpn.sh
